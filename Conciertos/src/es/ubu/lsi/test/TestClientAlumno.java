...
public class TestClientAlumno {
	...
	static void testService() throws Exception {
		...
		try {
			...
			// comprobar que se eliminan losregistros en la desactivación correcta
			desactivacionBorraRegistros(implService);
	...
	/**
	 * Comprueba que los registros asociados a un grupo desactivado han sido correctamente eliminados.
	 * 
	 * @param impService implementación del servicio
	 * @throws Exception
	 */
	private static void desactivacionBorraRegistros(Service impService) throws Exception {
		...
		try {
			System.out.println("Desactivar grupos borra los registros asociados");
			con = pool.getConnection();
			st = con.createStatement();
			rs = st.executeQuery("select * from concierto join compra on concierto.idconcierto = compra.idconcierto where concierto.idgrupo = 1");
			
			
			if(!rs.next()) {
				System.out.println("\tOK elimina correctamente todos los registros");
			} else {
				System.out.println("\tERROR no elimina todos los registros asociados al grupo");
			}
		  con.commit();
		} catch (Exception ex) {
